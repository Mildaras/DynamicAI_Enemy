using UnityEngine;
public class ExploderLaunchProjectile : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (t >= 1f)
        {
            // Spawn actual exploder
            if (exploderPrefab != null)
                Object.Instantiate(exploderPrefab, targetPosition, Quaternion.identity);
            Destroy(gameObject);
        }
    }
}